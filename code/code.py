...
def check_valid_bit(instr_no, address, cycle_no):
    if(True):  # assuming valid bit is always true
        time.sleep(1)
        print("Instruction ", instr_no,
              ": valid bit is true. Executed in cycle no. ", cycle_no)
        return True


def get_data_from_cache(instr_no, address, cycle_no):
    set_no = (address // 4) % 4
    tag = address // 16
    ...
    for key in cacheMemory[set_no]:
        if((key // 16) == tag):
            ...
            print("Instruction", instr_no, ": Data in cache is: " +
                  str(cacheMemory[set_no][key]), ". Executed in cycle no. ", cycle_no)
            break


def data_to_cpu(instr_no, cycle_no):
    time.sleep(1)
    print("Instruction ", instr_no, ": Data mentioned in the last step has been given to CPU. ",
          "Executed in cycle no. ", cycle_no)
...
def non_pipelined_cache():
    ...
    cycle_no = time.time()
    while n < n_instr:
        check_valid_bit(n+1, addresses[n], cycle_no)
        get_data_from_cache(n+1, addresses[n], cycle_no)
        data_to_cpu(n+1, cycle_no)
        n += 1
        print()
...
def pipelined_cache():
    ...
    while n < n_instr + 2:
        ...
        if(n > 0 and n < n_instr + 1):
            ...
            f2 = 1
        if(n > 1):
            t3 = threading.Thread(target=data_to_cpu, args=(n-1, n + 1))
            t3.start()
            f3 = 1
